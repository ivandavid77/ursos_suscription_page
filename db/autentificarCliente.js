module.exports = function(pool, config) {
    return function(usuario, password, callback) {
        pool.getConnection(function(err, conn) {
            ...
            conn.query(`
                SELECT
                    ...
                    apellido_paterno_encargado,
                    apellido_materno_encargado,
                    nombre_encargado,
                    palabra_secreta,
                    pregunta_secreta,
                ...
                function(err, rows) {
                    ...
                    if (rows.length == 0) {
                        conn.release();
                        callback('Verifique sus datos de acceso');
                        return;
                    }
                    var row = rows[0];
                    var cliente = {
                        ...
                        apellido_paterno_encargado: row.apellido_paterno_encargado,
                        apellido_materno_encargado: row.apellido_materno_encargado,
